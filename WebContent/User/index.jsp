...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
	    ...
	    <c:forEach items="${managers }" var="s">
		<tr>
			<td>${s.id }</td>
			<td>
				<a title="点击查看详情" href="${pageContext.request.contextPath }/manager/toShow/${s.id }">
					${s.userName }
				</a>
			</td>
			<td>${s.schId }</td>
			...
			<td>${s.createUser }</td>
			<td>
				 <a class="option-button" href="${pageContext.request.contextPath }/manager/toEdit/${s.id }">编辑</a>
				|<a class="option-button" onclick="del(${s.id});">删除</a>
				|<a class="option-button" href="${pageContext.request.contextPath }/manager/toShow/${s.id }">查看</a>
			</td>
		</tr>
	</c:forEach>
</table>
...
<script>
    $(function () {
		$('#addnew').click(function(){
				window.location.href="manager/toAdd";
		 });
    });
