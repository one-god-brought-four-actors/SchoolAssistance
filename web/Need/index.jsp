...
<body><% if(session.getAttribute("user") == null) response.sendRedirect("login.html"); %>
	...
	<div class="inline pull-right page">
		...
		<c:choose>
			...
			<c:otherwise>
				<a href='NeedListServlet?index=1'>首页</a>
				<a href='NeedListServlet?index=${page.index - 1}'>上一页</a>
			</c:otherwise>
		</c:choose>
		...
		<c:forEach var="p" begin="1" end="${page.getTotalPage() }">
			<c:choose>
				...
				<c:otherwise>
					<a href="NeedListServlet?index=${p }">${p }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			...
			<c:otherwise>
				<a href='NeedListServlet?index=${page.index + 1 }'>下一页</a>
				<a href='NeedListServlet?index=${page.getTotalPage() }'>尾页</a>
			</c:otherwise>
		</c:choose>
	</div>
</body>
